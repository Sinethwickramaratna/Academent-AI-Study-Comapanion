...
const timeSlots = Array.from({ length: HOURS_IN_DAY }, (_, hour) => formatHourLabel(hour));
const DAY_TIMELINE_HEIGHT = HOURS_IN_DAY * HOUR_ROW_HEIGHT;
...
const timeToMinutes = (time = '00:00') => {
  const [hours = 0, mins = 0] = String(time).split(':').map(Number);
...
const minutesToTime = (minutes) => {
  const safeMinutes = Math.min(MINUTES_IN_DAY - 1, Math.max(0, Math.round(minutes)));
  const hours = Math.floor(safeMinutes / 60);
  const mins = safeMinutes % 60;
  return `${String(hours).padStart(2, '0')}:${String(mins).padStart(2, '0')}`;
};
const formatTimeOption = (time = '00:00') => {
...
const buildTimedEventLayout = (dayEvents, date) => {
  ...
  return groups.flatMap((group) => {
    ...
    return positioned.map((item) => ({
      ...
      height: Math.max(((item.end - item.start) / 60) * HOUR_ROW_HEIGHT, MIN_TIMED_EVENT_HEIGHT),
      left: (item.lane / laneCount) * 100,
      width: 100 / laneCount,
      key: `${getEventId(item.event)}-${item.index}`,
    }));
  });
};
...
const formatDatePickerValue = (value) => {
  ...
  return date ? new Intl.DateTimeFormat('en', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }).format(date) : 'Choose date';
};
...
function PlannerDatePicker({ label, value, min, onChange }) {
  const selectedDate = fromDateInput(value);
  const minDate = fromDateInput(min);
  ...
  return (
    <div className={`planner-field planner-picker-field planner-date-picker ${open ? 'is-open' : ''}`} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false); }} onKeyDown={(event) => { if (event.key === 'Escape') setOpen(false); }}>
      {label && <span className="planner-field-label">{label}</span>}
      <button type="button" className={`planner-picker-trigger ${value ? 'has-value' : ''} ${open ? 'is-open' : ''}`} aria-haspopup="dialog" aria-expanded={open} onClick={toggleOpen}>
        <span className="material-symbols-outlined">calendar_month</span>
        <span><strong>{formatDatePickerValue(value)}</strong><small>{selectedDate ? formatMonth(selectedDate) : 'Select a day'}</small></span>
        <span className="material-symbols-outlined">expand_more</span>
      </button>
      {open && <div className="planner-picker-menu planner-date-menu" role="dialog" aria-label={`${label || 'Date'} calendar`}>
        <div className="planner-picker-menu-header">
          <button type="button" aria-label="Previous month" onClick={() => setViewDate((date) => new Date(date.getFullYear(), date.getMonth() - 1, 1))}><span className="material-symbols-outlined">chevron_left</span></button>
          <strong>{formatMonth(viewDate)}</strong>
          <button type="button" aria-label="Next month" onClick={() => setViewDate((date) => new Date(date.getFullYear(), date.getMonth() + 1, 1))}><span className="material-symbols-outlined">chevron_right</span></button>
        </div>
        <div className="planner-date-weekdays">{weekDays.map((day) => <span key={day}>{day.slice(0, 2)}</span>)}</div>
        <div className="planner-date-grid">
          {days.map((day) => {
            const disabled = minTime !== null && startOfDay(day.date).getTime() < minTime;
            const selected = selectedDate && sameDay(day.date, selectedDate);
            const classes = ['planner-date-option', day.currentMonth ? '' : 'is-muted', day.today ? 'is-today' : '', selected ? 'is-selected' : '', disabled ? 'is-disabled' : ''].filter(Boolean).join(' ');
            return <button type="button" key={day.key} className={classes} disabled={disabled} aria-pressed={Boolean(selected)} onClick={() => pickDate(day.date)}><span>{day.date.getDate()}</span></button>;
          })}
        </div>
        <div className="planner-date-menu-footer">
          <button type="button" onClick={() => setViewDate(startOfMonth(new Date()))}>This month</button>
          <button type="button" disabled={minTime !== null && startOfDay(new Date()).getTime() < minTime} onClick={() => pickDate(new Date())}>Today</button>
        </div>
      </div>}
    </div>
  );
...
function NotesTree({ data, selectedNoteIds, selectedPdfIds, onToggle }) {
  ...
  const renderMaterial = (item, type, context) => {
    ...
    return <label className={checked ? 'planner-material-row is-selected' : 'planner-material-row'} key={`${type}-${id}`}><input type="checkbox" checked={checked} onChange={() => onToggle({ item, type, ...context })} /><span className="material-symbols-outlined">{type === 'note' ? 'description' : 'picture_as_pdf'}</span><strong>{titleOrFallback(item, type === 'note' ? 'Untitled note' : 'Untitled PDF')}</strong></label>;
  };
...
function StudyPlannerPage({ profile, currentUser }) {
  ...
  const handleSave = async (submitEvent) => {
    ...
    setSaving(true);
    try {
      await saveStudyPlannerEvent(uid, { ...form, status: form.status || 'pending', durationMinutes: Number(form.durationMinutes || 0) }, editingId);
      setToast({ type: 'success', message: editingId ? 'Event and reminders updated.' : 'Event saved to your planner.' });
  ...
  const renderTimedEventBlock = ({ event, top, height, left, width, key }, density = 'day') => {
    ...
    return (
      <button
        key={key}
        type="button"
        className={`planner-timed-event ${density === 'week' ? 'week' : ''} ${event.status === 'completed' ? 'completed' : ''}`}
        style={{
      ...
      {weekDates.map((date) => {
        ...
        return (
          <div className="planner-week-day-column" key={`week-column-${date.toISOString()}`}>
            ...
            <div className="planner-week-events-layer">{layouts.map((layout) => renderTimedEventBlock(layout, 'week'))}</div>
          </div>
        );
      })}
    </div>
    ...
    return (
      <div className="planner-day-view" style={{ '--timeline-height': `${DAY_TIMELINE_HEIGHT}px` }}>
        ...
        <div className="planner-day-timeline">
          {timeSlots.map((slot) => <button type="button" aria-label={`Schedule ${slot} on ${formatDay(currentDate)}`} className="planner-day-dropzone" key={`day-slot-${slot}`} onClick={() => openCreateModal(currentDate, slot)} />)}
          <div className="planner-day-events-layer">{layouts.map((layout) => renderTimedEventBlock(layout))}</div>
        </div>
      </div>
    );
  };
