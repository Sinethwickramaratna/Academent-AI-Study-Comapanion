...
const getCardLearningState = (card) => {
  const mastery = Number(card?.masteryLevel || 0);
  const reviewCount = Number(card?.reviewCount || 0);
  const due = toDate(card?.nextReview);
  const now = new Date();

  if (mastery >= 80) return { label: 'Learned', icon: 'verified', tone: 'learned' };
  if (due && due <= now) return { label: 'Due', icon: 'event_repeat', tone: 'due' };
  if (reviewCount > 0) return { label: 'Learning', icon: 'school', tone: 'learning' };
...
function DeleteCollectionModal({ collection, working, onCancel, onConfirm }) {
  const cardCount = collection?.analytics?.totalFlashCards || collection?.cardCount || 0;
  return <div className="flash-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) onCancel(); }}><section className="flash-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="flash-delete-title"><header className="flash-confirm-header"><span className="material-symbols-outlined">delete</span><div><p>Delete collection</p><h3 id="flash-delete-title">{collection?.title || 'Flash card collection'}</h3></div></header><div className="flash-confirm-body"><p>This will permanently remove the collection and its {cardCount} flash cards from your library.</p></div><footer className="flash-modal-actions"><button className="flash-button flash-button--ghost" type="button" disabled={working} onClick={onCancel}>Cancel</button><button className="flash-button flash-button--danger" type="button" disabled={working} onClick={onConfirm}><span className="material-symbols-outlined">{working ? 'hourglass_top' : 'delete'}</span>{working ? 'Deleting...' : 'Delete'}</button></footer></section></div>;
}
...
function FlashCardRow({ card, activeCollection, onStudy, onView }) {
  const mastery = Math.max(0, Math.min(100, Number(card.masteryLevel || 0)));
  const learningState = getCardLearningState(card);
  const difficulty = card.difficulty || 'mixed';
  const sourceTitles = card.sourceTitles?.length ? card.sourceTitles : activeCollection.selectedSources?.map((source) => source.title) || [];
  const sourceText = sourceTitles.join(', ') || 'Generated from selected study sources';
  const chips = [...new Set([...(card.tags || []), ...(card.keywords || [])].filter(Boolean))];
  const dueLabel = formatDueDate(card.nextReview);
  const lastReviewedLabel = formatDate(card.lastReviewed, 'never');
  const stats = [
    { label: 'Next Review', value: dueLabel },
    { label: 'Interval', value: `${card.interval || 0}d` },
    { label: 'Ease', value: Number(card.easeFactor || 2.5).toFixed(2) },
    { label: 'Reviews', value: card.reviewCount || 0 },
    { label: 'Mastery', value: `${mastery}%` },
    { label: 'Type', value: typeLabel(card.type) },
  ];
  const openCard = () => onView(card);

  return <article className="flash-card-row" role="button" tabIndex={0} onClick={(event) => { if (!event.target.closest('button, a, input, select, textarea')) openCard(); }} onKeyDown={(event) => { if ((event.key === 'Enter' || event.key === ' ') && !event.target.closest('button, a, input, select, textarea')) { event.preventDefault(); openCard(); } }} title="Click to view card details">
    <div className="flash-card-row__top">
      <span className="flash-pin" aria-hidden="true"><span className="material-symbols-outlined">style</span></span>
      <div className="flash-card-front">
        <strong>{card.front}</strong>
        <small>{sourceText}</small>
        {chips.length > 0 && <div className="flash-card-tags">{chips.map((chip) => <span key={chip}>{chip}</span>)}</div>}
      </div>
      <div className="flash-row-actions">
        <button type="button" aria-label="Study this collection" onClick={onStudy}><span className="material-symbols-outlined flash-symbol--white">play_arrow</span></button>
        <button type="button" aria-label="View card details" onClick={openCard}><span className="material-symbols-outlined">visibility</span></button>
      </div>
    </div>
    <div className="flash-card-row__meta">
      <span className={`flash-difficulty flash-difficulty--${difficulty}`}>{labelCase(difficulty)}</span>
      <span className="flash-ai-badge"><span className="material-symbols-outlined flash-symbol--white flash-symbol--dark-light">auto_awesome</span>{card.createdBy || 'AI'}</span>
      <span className={`flash-learning-status flash-learning-status--${learningState.tone}`}><span className="material-symbols-outlined">{learningState.icon}</span>{learningState.label}</span>
      <div className="flash-review-status"><strong>{dueLabel}</strong><small>Last reviewed {lastReviewedLabel}</small></div>
    </div>
    <div className="flash-srs-grid">{stats.map((stat) => <span key={stat.label}>{stat.label} <strong>{stat.value}</strong></span>)}</div>
    <div className="flash-card-mastery" style={{ '--mastery': `${mastery}%` }}><div><span>Progress</span><strong>{mastery}%</strong></div><div><span /></div></div>
  </article>;
}
function FlashCardsWorkspace({ profile, currentUser }) {
  ...
  const confirmRemoveCollection = async () => { if (!collectionPendingDelete) return; const collectionId = collectionPendingDelete.id; await flash.removeCollection(collectionId); if (activeCollectionId === collectionId) setActiveCollectionId(null); setCollectionPendingDelete(null); };
  ...
  return <main className="p-gutter md:p-margin-desktop space-y-xl flash-page">
    ...
    <section className="flash-header">
      ...
      <div className="flash-header-actions">
        ...
        <button className="flash-button flash-button--primary" type="button" onClick={() => setIsGeneratorOpen(true)}><span className="material-symbols-outlined flash-symbol--white flash-symbol--dark-light">auto_awesome</span>Create Flash Cards</button>
      </div>
    </section>
    ...
    {collectionPendingDelete && <DeleteCollectionModal collection={collectionPendingDelete} working={flash.working} onCancel={() => setCollectionPendingDelete(null)} onConfirm={confirmRemoveCollection} />}
  </main>;
}
