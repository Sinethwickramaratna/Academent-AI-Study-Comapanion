  ...
  return <article className="flash-card-row" role="button" tabIndex={0} onClick={(event) => { if (!event.target.closest('button, a, input, select, textarea')) openCard(); }} onKeyDown={(event) => { if ((event.key === 'Enter' || event.key === ' ') && !event.target.closest('button, a, input, select, textarea')) { event.preventDefault(); openCard(); } }} title="Click to view card details">
    <div className="flash-card-row__top">
      ...
      <div className="flash-row-actions">
        <button type="button" aria-label="Study this collection" onClick={onStudy}><span className="material-symbols-outlined">play_arrow</span></button>
    ...
    <div className="flash-card-row__meta">
      ...
      <span className="flash-ai-badge"><span className="material-symbols-outlined">auto_awesome</span>{card.createdBy || 'AI'}</span>
